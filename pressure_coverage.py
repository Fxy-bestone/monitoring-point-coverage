...
results = sim.run_sim()
pressure_at_5hr = results.node['pressure'].loc[:,['10','15']]
...
for junction_name, junction in wn.junctions():    
    ...
    pred_list = pred_list.append(pressure_at_5hr)
    junction.demand_timeseries_list[0].base_value = junction.demand_timeseries_list[0].base_value/1.1
    pred0.append(junction_name)
    
    
#pred_list0=pred_list.columns = ['One','Two','Three']
#pred_list.columns = ['c','b']
pred_list.index=pred0
...
index=[]
for i in range(len(pred_list)):
    if sum(pred_list.iloc[i,:]<2)==2:
        index.append(pred_list.index[i])
        
percent=1-len(index)/len(pred_list)
print(index)
print(percent)

#pred_list.columns.values.tolist()



#b = sum(pred_list.iloc[1,:]<2)==2
